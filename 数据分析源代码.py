...
# Plot the bars for each group of dates, using different shades of blue
plt.bar(r1, max_wind_speed_array[0,:], color='royalblue', width=bar_width, label=dates[0])
plt.bar(r2, max_wind_speed_array[1,:], color='cornflowerblue', width=bar_width, label=dates[1])
plt.bar(r3, max_wind_speed_array[2,:], color='lightsteelblue', width=bar_width, label=dates[2])
    ...
    for j in range(8):
        ...
        if i == 0:
            ...
            for k in range(5):
                ...
                total_duration = len(data[data>3])/len(data)*24
...
r3 = [x + bar_width for x in r2]
# Create a bar plot
plt.bar(r1, total_duration_array[0,:], color='royalblue', width=bar_width, label=dates[0])
plt.bar(r2, total_duration_array[1,:], color='cornflowerblue', width=bar_width, label=dates[1])
plt.bar(r3, total_duration_array[2,:], color='lightsteelblue', width=bar_width, label=dates[2])
...
# %%
# print all the greatly gusty days
print(group_1)

# %%
#Task Task 8: Retrieve the probability density function of wind speed for1900greatlw gusty daw and wisualize themin one single figure
...
# loop over all greatly gusty days to retrieve the probability desity function of wind speed for every greatly gusty day and plot them in one single figure
for i in range(len(group_1)):
    # the first date is Nov 4th, which is 20xx1104
    file_name = f"D:/电子版作业/大气探测原理/期末大作业/Nov/{group_1[i][0]}11{group_1[i][1]:02d}/WSAVG2M_{group_1[i][0]}11{group_1[i][1]:02d}.txt"
    # Read the data from the file, which is a txt file
    with open(file_name, 'r') as f:
        data = f.read()
    # Check if the file is empty
    ...
    data = pd.to_numeric(data, errors='coerce')
    # Calculate and plot the PDF of the data
    sns.kdeplot(data, ax=ax, label=f'{group_1[i][0]}/11/{group_1[i][1]:02d}', bw_adjust=1, cut=0)
...
# %%
# Task 9: Extract the median and maximum wind speed for each greatly gusty day and visualize them in one single figure
# first we create a 2×15 array to store the median and maximum wind speed for each greatly gusty day
median_max_array = np.zeros((2, 15))

# loop over all greatly gusty days to extract the median and maximum wind speed for each greatly gusty day and store them in the array
for i in range(len(group_1)):
    # the first date is Nov 4th, which is 20xx1104
    file_name = f"D:/电子版作业/大气探测原理/期末大作业/Nov/{group_1[i][0]}11{group_1[i][1]:02d}/WSAVG2M_{group_1[i][0]}11{group_1[i][1]:02d}.txt"
    # Read the data from the file, which is a txt file
    ...
    data = pd.to_numeric(data, errors='coerce')
    # Calculate the median and maximum wind speed for each greatly gusty day
    median = data.median()
    maximum = data.max()
    # Store the median and maximum wind speed for each greatly gusty day in the array
    median_max_array[0, i] = median
    median_max_array[1, i] = maximum

# Use the mean values of median and maximum wind speed of Nov 4th over 8 years as the reference values
# Calculate the mean values of median and maximum wind speed of Nov 4th over 8 years
# first we create a 2×8 array to store the median and maximum wind speed of Nov 4th over 8 years
median_max_array_2 = np.zeros((2, 8))
for i in range(8):
    # the first date is Nov 4th, which is 20xx1104
    file_name = f"D:/电子版作业/大气探测原理/期末大作业/Nov/{2016+i}11{4:02d}/WSAVG2M_{2016+i}11{4:02d}.txt"
    # Read the data from the file, which is a txt file
    with open(file_name, 'r') as f:
        data = f.read()
    # Check if the file is empty
    ...
    data = pd.to_numeric(data, errors='coerce')
    # Calculate the median and maximum wind speed of Nov 4th over 8 years
    median = data.median()
    maximum = data.max()
    # Store the median and maximum wind speed of Nov 4th over 8 years in the array
    median_max_array_2[0, i] = median
    median_max_array_2[1, i] = maximum

# Calculate the mean values of median and maximum wind speed of Nov 4th over 8 years
median_mean = median_max_array_2[0, :].mean()
maximum_mean = median_max_array_2[1, :].mean()

# Use a histogram to visualize the median and maximum wind speed for each greatly gusty day in one single figure
# Note that we use one bar with upper and lower parts in different colors to illustrate the median and maximum wind speed for each greatly gusty day
# Each bar should be separated from the next bar by a small gap
# Initialize a matplotlib figure and axes
# We don't use subplots, just one figure to include 15 bars with upper and lower parts in different colors
...
df = pd.DataFrame(median_max_array.T, columns=['Median', 'Max'])

# Assuming group_1[i][0] represents year and group_1[i][1] represents day
df['Date'] = [f'{group_1[i][0]}/11/{group_1[i][1]:02d}' for i in df.index]

# Set 'Date' as the new index
df.set_index('Date', inplace=True)

# Plot the median wind speed
sns.set_color_codes("pastel")
bar1 =sns.barplot(x=df.index, y="Median", data=df, label="Median", color="b")

# Subtract the median from the maximum and plot the result
df['Max_minus_Median'] = df['Max'] - df['Median']
sns.set_color_codes("muted")
bar2 = sns.barplot(x=df.index, y="Max_minus_Median", data=df, label="Max", color="b", bottom=df["Median"])

# Include the reference values
plt.axhline(y=median_mean, color='r', linestyle='-', label='Median Mean of Nov 4th')
plt.axhline(y=maximum_mean, color='r', linestyle='--', label='Maximum Mean of Nov 4th')

# Add a legend and informative axis label(loc="lower left" means the lower left corner of legend is used to locate itself, 
# while bbox_to_anchor=(0, 1) means to locate the legend at the upper left corner of the axes)
ax.legend(ncol=2, loc="lower left", frameon=True, bbox_to_anchor=(0, 1), prop={'size': 10})
ax.set(ylabel="Wind Speed(m/s)",xlabel=None)
...
# %%
# Caculate the mean median and maximum wind speed over 15 greatly gusty days
Mean_greatly_gusty_median = median_max_array[0, :].mean()
Mean_greatly_gusty_maximum = median_max_array[1, :].mean()

# We create a latex table to show the exact values of median and maximum wind speed for each greatly gusty day,using the package 'tabulate'
# First we import the package 'tabulate'
from tabulate import tabulate

# Then we create a 15×3 array to store the exact values of median and maximum wind speed for each greatly gusty day
# Using values form median_max_array directly
table = np.zeros((15, 4))
for i in range(15):
    table[i, 0] = group_1[i][0]
    table[i, 1] = group_1[i][1]
    table[i, 2] = median_max_array[0, i]
    table[i, 3] = median_max_array[1, i]

#print(tabulate(table, tablefmt="latex"))


# %%
# print the mean median and maximum wind speed over 15 greatly gusty days and Nov 4th over 8 years
print(f"Mean median of 15 greatly gusty days: {Mean_greatly_gusty_median:.2f} m/s")
print(f"Mean maximum of 15 greatly gusty days: {Mean_greatly_gusty_maximum:.2f} m/s")
print(f"Mean median of Nov 4th over 8 years: {median_mean:.2f} m/s")
print(f"Mean maximum of Nov 4th over 8 years: {maximum_mean:.2f} m/s")

# %%
# Task 10: Categorize the wind speed for each greatly gusty day according to the Beaufort scale into percentages and visualize the results
# let's define the Beaufort scale first: we only introduce scales from 0 to 6, where v(m/s) as the value of wind speed
# Scale 0: 0<=v<0.25 m/s
# Scale 1: 0.25<=v<1.55 m/s
# Scale 2: 1.55<=v<3.35 m/s
# Scale 3: 3.35<=v<5.45 m/s
# Scale 4: 5.45<=v<7.95 m/s
# Scale 5: 7.95<=v<10.75 m/s
# Scale 6: 10.75<=v<13.85 m/s

# First we create a 7×15 array to store the percentages of wind speed for each greatly gusty day
# The first dimension represents the 7 scales, the second dimension represents the 15 greatly gusty days
# Each element in the array represents the percentage of wind speed for a specified scale for a specified greatly gusty day
percentage_array = np.zeros((7, 15))

# loop over all greatly gusty days to categorize the wind speed for each greatly gusty day according to the Beaufort scale into percentages and store them in the array
for i in range(len(group_1)):
    # the first date is Nov 4th, which is 20xx1104
    file_name = f"D:/电子版作业/大气探测原理/期末大作业/Nov/{group_1[i][0]}11{group_1[i][1]:02d}/WSAVG2M_{group_1[i][0]}11{group_1[i][1]:02d}.txt"
    # Read the data from the file, which is a txt file
    ...
    data = pd.to_numeric(data, errors='coerce')
    n = data.size
    # Categorize the wind speed for each greatly gusty day according to the Beaufort scale into percentages
    # Scale 0: 0<=v<0.25 m/s
    percentage_array[0, i] = data[data < 0.25].size/n
    # Scale 1: 0.25<=v<1.55 m/s
    percentage_array[1, i] = data[(data >= 0.25) & (data < 1.55)].size/n
    # Scale 2: 1.55<=v<3.35 m/s
    percentage_array[2, i] = data[(data >= 1.55) & (data < 3.35)].size/n
    # Scale 3: 3.35<=v<5.45 m/s
    percentage_array[3, i] = data[(data >= 3.35) & (data < 5.45)].size/n
    # Scale 4: 5.45<=v<7.95 m/s
    percentage_array[4, i] = data[(data >= 5.45) & (data < 7.95)].size/n
    # Scale 5: 7.95<=v<10.75 m/s
    percentage_array[5, i] = data[(data >= 7.95) & (data < 10.75)].size/n
    # Scale 6: 10.75<=v<13.85 m/s
    percentage_array[6, i] = data[(data >= 10.75) & (data < 13.85)].size/n
...
# %%
# Visualize the percentage of wind speed for each greatly gusty day
# We use stack bar chart to visualize the percentage of wind speed for each greatly gusty day, with 100% as the final equal total height
# We use matplotlib and CSS colors
# Color-Scale relations: lightsteelblue-0, skyblue-1, deepskyblue-2, cornflowerblue-3, dodgerblue-4, royalblue-5, blue-6
...
# Create a colormap
cmap = ListedColormap(['lightsteelblue', 'skyblue', 'deepskyblue', 'cornflowerblue', 'dodgerblue', 'royalblue', 'blue'])

# Use the percentage_array to create a pandas DataFrame
df = pd.DataFrame(percentage_array.T, columns=['Scale 0', 'Scale 1', 'Scale 2', 'Scale 3', 'Scale 4', 'Scale 5', 'Scale 6'])
...
df.plot.bar(stacked=True, colormap=cmap, ax=ax)

# Set the labels of the x and y axes
ax.set(ylabel="Percentage of Wind Speed(%)",xlabel=None)

# Rescale ytickes from 0 to 100%
